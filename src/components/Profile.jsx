...
const Profile = () => {
    ...
    const TfaConfig = ({ title, method }) => {
        ...
        // Special handling for email - cannot be disabled
        const isEmailMethod = method === "email";
        const canDisable = config && !isEmailMethod;
                ...
                {config && (
                    <div className="security-item-disable">
                        {canDisable ? (
                            ...
                            <p
                                className="text-muted security-link text-decoration-none"
                                style={{
                                    opacity: 0.5,
                                    cursor: "not-allowed",
                                    fontSize: "12px",
                                ...
                                title={
                                    isEmailMethod
                                        ? "Email 2FA is required and cannot be disabled"
                                        : "Cannot disable"
                                }
                            >
                                {isEmailMethod ? "Required" : "Cannot disable"}
                            </p>
                        )}
                    </div>
                )}
                ...
                <main className="profile-page">
                    <TwoFactorModal
                        ...
                        onResult={async (res) => {
                            ...
                            if (res) {
                                console.log(
                                    "✅ 2FA operation successful, refreshing user data",
                                );

                                try {
                                    // Force refresh the user data with network-only fetch policy
                                    await refetch();
                                    console.log(
                                        "✅ User data refetched successfully",
                                    );
                                    ...
                                    setErrorModal({
                                        ...
                                        title: "Warning",
                                        message:
                                            "2FA was updated but there was an issue refreshing the page. Please refresh manually.",
                                    });
                                }
                            } else {
                                console.log("❌ 2FA operation failed");
                                setErrorModal({
                                    isOpen: true,
                                    title: "Error",
                                    message:
