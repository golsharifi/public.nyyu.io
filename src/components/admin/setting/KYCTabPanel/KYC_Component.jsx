...
import { update_KYC_Setting } from "../../../../store/actions/kycSettingAction";
...
const KYCComponent = ({ icon, prop, topic, thresholds = {} }) => {
    ...
    const InitialRequirement = thresholds[prop]
        ? requirementsList[0]
        : requirementsList[1];
    const [requirement, setRequirement] = useState(InitialRequirement);

    const [threshold, setThreshold] = useState(
        !thresholds[prop] ? "" : thresholds[prop],
    );
    const thresholdError = useMemo(() => {
        if (requirement.value === "required" && !threshold)
                ...
                <form className="form" onSubmit={(e) => e.preventDefault()}>
                    ...
                    <div className="input_div">
                        ...
                        <div className="input">
                            <p
                                className={
                                    requirement.value === "not_required"
                                        ? "disabled"
                                        : ""
                                }
                                style={{ fontSize: 12 }}
                            ...
                            <NumberFormat
                                className={`black_input ${requirement.value === "not_required" ? "disabled" : ""}`}
                                disabled={
                                    requirement.value === "not_required"
                                        ? true
                                        : false
