...
const WalletBalance = () => {
    ...
    const error = useMemo(() => {
        ...
        for (let i = 0; i < stakings.length; i++) {
            ...
            if (!stakings[i].ratio)
                return { index: i, item: "ratio", desc: "Input is required" };
        }
        return {};
    }, [stakings]);

    const openEditModal = () => {
        const staking = tasks.staking.map((item) => ({
    ...
    const handleSubmit = async () => {
        ...
        const updateData = {
            ...
            wallet: tasks.wallet.map((item) => ({
                amount: Number(item.amount),
                point: Number(item.point),
            })),
            auction: tasks.auction,
            direct: tasks.direct,
            staking: stakings.map((item) => ({
                expiredTime: item.expiredTime,
                ratio: item.ratio,
            })),
        };
                ...
                <Main>
                    <UnitRow>
                        <div
                            ...
                            data-bs-target="#ndb_token"
                            onClick={() => setShow(!show)}
                            onKeyDown={() => setShow(!show)}
                            aria-hidden="true"
                        >
                            <p>
                                Ndb token staking in the wallet{" "}
                        ...
                        <div className="points">
                            <p>
                                {tasks.staking[0]?.ratio} * x tokens *{" "}
                                {tasks.staking[0]?.expiredTime}
                            </p>
                        </div>
                        <div className="edit">
                            <p>
                        ...
                        {tasks.staking.map((value, index) => {
                            ...
                            return (
                                <UnitRow key={index}>
                                    ...
                                    <div className="threshold">
                                        <p key={index}>
                                            {value.expiredTime} Days
                                        </p>
                                    </div>
                                    <div className="points">
                                        <p key={index}>
                                            {value.ratio} * x tokens *{" "}
                                            {value.expiredTime}
                                        </p>
                                    </div>
                        ...
                        return (
                            <UnitRowForMobile key={index}>
                                <div className="left">
                                    ...
                                    >
                                        X tokens in {value.expiredTime} days
                                        staking
                                    </p>
                                    <p
                                        style={{
                    ...
                    {stakings.map((value, index) => {
                        return (
                            <div key={index} className="input">
                                <div className="input_div">
                                    <NumberFormat
                                        className={`black_input ${showError && error.index === index && error.item === "expiredTime" ? "error" : ""}`}
                                        placeholder="Enter number"
                                        thousandSeparator={true}
                                        allowNegative={false}
                                        value={value.expiredTime}
                                        onValueChange={(values) => {
                                            stakings[index].expiredTime =
                                                values.value;
                                ...
                                <div className="input_div">
                                    <NumberFormat
                                        className={`black_input ${showError && error.index === index && error.item === "ratio" ? "error" : ""}`}
                                        placeholder="Enter number"
                                        thousandSeparator={true}
                                        allowNegative={false}
                                        value={value.ratio}
                                        onValueChange={(values) => {
                                            stakings[index].ratio =
                                                values.value;
                                            setStakings([...stakings]);
                                        }}
                                    />
