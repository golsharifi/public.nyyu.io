...
import {
    ...
    GET_PAYPAL_DEPOSIT_TRANSACTIONS_BY_USER, // ← Correct name
    GET_PRESALE_ORDERS_BY_USER,
    GET_STRIPE_DEPOSIT_TX_BY_USER, // ← Correct name
...
const TransactionsProvider = ({ children }) => {
    ...
    const [bankDepositTransactions, setBankDepositTransactions] =
        useState(null);
    const [bankWithdrawTransactions, setBankWithdrawTransactions] =
        useState(null);
    ...
    useQuery(GET_PAYPAL_DEPOSIT_TRANSACTIONS_BY_USER, {
        variables: {
            showStatus: showStatus.deposit,
        },
        onCompleted: (data) => {
            const fooList = _.orderBy(
                data.getPaypalDepositTxnsByUser,
                ["createdAt"],
                ["desc"],
            ).map((item) => {
                ...
                return {
                    ...
                    fee: item.fee || 0,
                    status: item.status,
                    amount: item.usdAmount || 0,
                    deposited: item.usdAmount || 0,
                    type: "PayPal Deposit",
        ...
        onCompleted: (data) => {
            const fooList = _.orderBy(
                ...
                return {
                    ...
                    deposited: item.usdAmount || 0,
                    type: "Stripe Deposit",
                    paymentId: item.fiatType + " payment",
                    asset: item.fiatType,
                    cryptoAsset: item.cryptoType,
                };
            });
    ...
    useQuery(GET_COINPAYMENT_DEPOSIT_TX_BY_USER, {
        ...
        onCompleted: (data) => {
            const fooList = _.orderBy(
                data.getCoinpaymentDepositTxByUser,
                ["createdAt"],
                ["desc"],
            ).map((item) => {
                const createdTime = new Date(item.createdAt);
    ...
    useQuery(GET_BANK_WITHDRAW_TRANSACTIONS_BY_USER, {
        ...
        onCompleted: (data) => {
            const list = _.orderBy(
                data.getBankWithdrawRequestsByUser,
                ["confirmedAt"],
                ["desc"],
            ).map((item) => {
