...
const forceCurrencyRates = async () => {
    ...
    // Method 3: Try Fixer.io (Premium) - with improved error handling
    if (process.env.GATSBY_FIXER_API_KEY) {
        try {
            ...
            const res = await fetch(
                `https://api.fixer.io/latest?access_key=${process.env.GATSBY_FIXER_API_KEY}&base=USD&format=1`,
                {
                    method: "GET",
                    headers: {
            ...
            if (res.ok) {
                ...
                if (data && data.success && data.rates) {
                    ...
                    console.warn(
                        "⚠️ FORCE: Fixer.io API issue:",
                        data.error?.info || "Invalid response",
                    );
                }
            } else {
                console.warn("⚠️ FORCE: Fixer.io HTTP error:", res.status);
            }
        } catch (err) {
            if (err.name === "AbortError") {
                console.warn("⚠️ FORCE: Fixer.io timeout");
            } else {
                console.warn("⚠️ FORCE: Fixer.io failed:", err.message);
            }
        }
    }
